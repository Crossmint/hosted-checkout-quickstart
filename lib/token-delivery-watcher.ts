...
/**
 * Detects the on-chain delivery of a fungible token (ERC-20) purchase on
 * Robinhood Chain.
 *
 * The hosted checkout runs on Crossmint's page (popup), so the demo can't
 * receive a callback when the order completes. Instead, we poll public RPCs
 * for an incoming ERC-20 Transfer into the prefilled recipient wallet. The
 * matching log gives us the real swap/mint transaction hash.
 */
  ...
  const logs = await rpcCall<JsonRpcLog[]>("eth_getLogs", [
    {
      ...
      toBlock: "latest",
      // Any ERC-20 Transfer into the recipient wallet. The sender topic is
      // left as null so we match transfers from any address (swap router,
      // treasury, LP, etc.).
      topics: [TRANSFER_TOPIC, null, paddedWallet],
    },
  ]);
  // ERC-20 Transfer has 3 topics; ERC-721 has 4.
  return logs.find((log) => log.topics.length === 3);
}

export interface UseTokenDeliveryWatcherOptions {
  recipientWalletAddress: string;
}

export function useTokenDeliveryWatcher({
  recipientWalletAddress,
}: UseTokenDeliveryWatcherOptions) {
  ...
  const startWatching = useCallback(() => {
    ...
    (async () => {
      try {
        const [blockNumberHex] = await Promise.all([
          // We already know the recipient wallet address, so no order quote is
          // needed; just grab the current block to start polling from.
          rpcCall<string>("eth_blockNumber", []),
        ]);
        // Small back-buffer so a fast payment isn't missed while starting.
        const fromBlock = Math.max(Number.parseInt(blockNumberHex, 16) - 2, 0);
        const deadline = Date.now() + WATCH_TIMEOUT_MS;

        while (!stoppedRef.current && Date.now() < deadline) {
          try {
            const log = await findDeliveryLog(recipientWalletAddress, fromBlock);
            if (log) {
              setDelivery({
  ...
  }, [recipientWalletAddress, closeCheckoutPopup]);
